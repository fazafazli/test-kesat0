...
const TABS = [
  {
    ...
    imageUrl: "/section4/pengenalan.webp",
  },
  {
    id: "jelajah",
...
function useAppearOnScroll(ref: React.RefObject<HTMLElement | null>) {
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        section.classList.toggle("animate-in", entry.isIntersecting);
      },
      { threshold: 0.1 },
    );
  ...
  return (
    ...
    >
      <Image
        ...
        fill
        unoptimized
        sizes="(max-width: 640px) 19vw, 8.5vw"
        className="bd-tab-bg"
        aria-hidden="true"
        draggable={false}
      />
...
export default function SectionEmpat() {
  ...
  const [activeId, setActiveId] = useState<TabId>("pengenalan");
