...
export const drawGround = (
  ...
  // FIX 1: Math.floor prevents sub-pixel rendering (anti-aliasing) 
  // which forces the CPU to work harder and adds subtle lag.
  const scrollX = Math.floor(-(offset % CANVAS_WIDTH));
  
  const cloudHeight = GROUND.height;
  const cloudY = CANVAS_HEIGHT - cloudHeight;

  if (cloudCache) {
    // FAST PATH: Use the off-screen canvas cache (No scaling math required!)
    ctx.drawImage(cloudCache, scrollX, cloudY);
    
    // FIX 2: Subtract 1 pixel (CANVAS_WIDTH - 1) to overlap them and kill the vertical seam!
    ctx.drawImage(cloudCache, scrollX + CANVAS_WIDTH - 1, cloudY);
    
  } else if (cloudImg && cloudImg.complete) {
    // SLOW PATH FALLBACK: Just in case the cache hasn't loaded yet
